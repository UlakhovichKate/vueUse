...
<template>
  ...
  <div>
    ...
    <form class="my-5" @submit.prevent="submit">
      <h3 class="mt-3 text-lg font-bold" v-text="stepper.current.value.title" />
      <div class="flex flex-col justify-center gap-2 mt-2">
        ...
        <div>
          <button v-if="!stepper.isLast.value" :disabled="!stepper.current.value.isValid()" class="btn btn-danger">
            Next
          </button>
          <button v-if="stepper.isLast.value" :disabled="!stepper.current.value.isValid()" class="btn btn-danger">
            Submit
